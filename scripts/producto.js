 (() => {
        ...
        function updateFavoriteButton(button, productId) {
          ...
          button.setAttribute('aria-label', `${active ? 'Quitar' : 'Añadir'} ${productName} ${active ? 'de' : 'a'} favoritos`);
        }
        ...
        function renderProduct() {
          ...
          document.querySelector('#product-meta-description').content = `${product.name}. Conocé sus medidas, materiales y garantía en Hermanos Jota Muebles.`;
          document.querySelector('#product-category').textContent = product.category;
          document.querySelector('#product-title').textContent = product.name;
          document.querySelector('#product-price').textContent = product.price;
          document.querySelector('#product-description').textContent = product.description;
          document.querySelector('#product-rating').textContent = product.rating;
          document.querySelector('#product-rating-count').textContent = `${product.ratingCount} calificaciones`;
        ...
        function renderReviews() {
          const reviews = [
            ['L', 'Lucía Ferrer', 5, `La calidad de ${product.name} se nota desde el primer momento. Llegó perfectamente embalado y la terminación es realmente muy cuidada.`],
            ['M', 'Mateo Ruiz', 5, `El diseño queda incluso mejor de lo que se aprecia en las fotos. ${product.name} se siente firme, cómodo y pensado para durar.`],
            ['C', 'Carolina Vega', 4, `Muy buena experiencia de compra y excelente atención. La pieza mantiene el equilibrio entre lo artesanal y lo moderno que estaba buscando.`]
          ];

          document.querySelector('#product-reviews-grid').innerHTML = reviews.map(([initial, name, rating, text]) => `
              ...
              <header class="review-card__header">
                <div class="review-card__user">
                  ...
                  <div>
                    ...
                    <span class="review-card__source">Comprador verificado</span>
                  </div>
                </div>
                <div class="review-card__rating" aria-label="Calificación ${rating} de 5 estrellas">
        ...
        function createRelatedCard([productId, relatedProduct]) {
          ...
          return `
            <article class="product-card" data-id="${productId}" data-category="${relatedProduct.categoryKey}">
              <header class="product-card__header">
                <div class="product-card__info">
                  <h3 class="product-card__title">${relatedProduct.name}</h3>
                  <p class="product-card__price">${relatedProduct.price}</p>
                </div>
              ...
              <div class="product-card__image-wrapper">
                <a href="./producto.html?id=${productId}" aria-label="Ver detalle de ${relatedProduct.name}">
                  <img src="${relatedProduct.image}" alt="${relatedProduct.name}" class="product-card__image" loading="lazy" width="300" height="220" />
                </a>
              </div>

              <div class="product-card__actions">
                <div class="product-card__primary-actions">
                  <button type="button" class="btn btn--primary" data-action="buy-now" data-id="${productId}">Comprar</button>
                  <button type="button" class="btn btn--gold" data-action="add-cart" data-id="${productId}" aria-label="Agregar ${relatedProduct.name} al carrito">
                ...
                <div class="product-card__secondary-actions">
                  <a href="./producto.html?id=${productId}" class="product-card__link-detail">Ver detalle</a>
                  <button type="button" class="btn--icon${favorite ? ' is-active' : ''}" data-action="toggle-fav" data-id="${productId}" aria-label="${favorite ? 'Quitar' : 'Añadir'} ${relatedProduct.name} ${favorite ? 'de' : 'a'} favoritos" aria-pressed="${favorite}">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true" focusable="false">
        ...
        function renderRelatedProducts() {
          const entries = Object.entries(products).filter(([productId]) => productId !== currentId);
          const sameCategory = entries.filter(([, item]) => item.category === product.category);
          const otherCategories = entries.filter(([, item]) => item.category !== product.category);
        ...
        thumbnails.forEach((thumbnail, index) => {
          thumbnail.addEventListener('click', () => {
            thumbnails.forEach((item) => item.classList.remove('product-gallery__thumb--active'));
            thumbnail.classList.add('product-gallery__thumb--active');
            mainImage.classList.remove('product-gallery__main-img--detail-top', 'product-gallery__main-img--detail-bottom');

            if (index === 1) mainImage.classList.add('product-gallery__main-img--detail-top');
            if (index === 2) mainImage.classList.add('product-gallery__main-img--detail-bottom');
          });
        });

        favoriteButton.addEventListener('click', () => toggleFavorite(currentId, favoriteButton));
        ...
        relatedGrid.addEventListener('click', (event) => {
          ...
          if (actionButton.dataset.action === 'buy-now') {
            window.location.href = `./producto.html?id=${productId}`;
          }

          if (actionButton.dataset.action === 'toggle-fav') {
